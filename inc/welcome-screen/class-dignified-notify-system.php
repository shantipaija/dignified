...
if ( ! class_exists( 'dignified_Notify_System' ) ) {
	/**
	 * Class dignified_Notify_System
	 */
	class dignified_Notify_System {
		/**
		 * @param $ver
		...
		public static function version_check( $ver ) {
			...
			return version_compare( $dignified['Version'], $ver, '>=' );
		}
		...
		public static function has_posts() {
			...
			$imported = get_option( 'dignified_importded_demo' );
			if ( $imported ) {
				return true;
			}
			...
			if ( $count_posts->publish > 4 ) {
				add_option( 'dignified_importded_demo', true );
				return true;
			}
		...
		public static function widget_importer_exporter_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: Widget Importer Exporter Plugin', 'dignified' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Activate: Widget Importer Exporter Plugin', 'dignified' );
			}
		...
		public static function wordpress_importer_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: WordPress Importer', 'dignified' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Activate: WordPress Importer', 'dignified' );
			}
		...
		public static function wordpress_importer_description() {
			$installed = self::check_plugin_is_installed( 'wordpress-importer' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress Importer to create the demo content.', 'dignified' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Importer to create the demo content.', 'dignified' );
			}
		...
		public static function widget_importer_exporter_description() {
			$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress widget importer to create the demo content', 'dignified' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'dignified' );
			}
