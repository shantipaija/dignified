<?php
/**
 * dignified functions and definitions
 *
 * @package dignified
 */
 ...
 */
function dignified_content_width()
{
...
add_action('template_redirect', 'dignified_content_width');

if (! function_exists('dignified_main_content_bootstrap_classes')) :
    /**
 * Add Bootstrap classes to the main-content-area wrapper.
 */
    function dignified_main_content_bootstrap_classes()
    {
...
endif; // dignified_main_content_bootstrap_classes

if (! function_exists('dignified_setup')) :
    /**
 ...
 */
    function dignified_setup()
    {
           ...
           */
        load_theme_textdomain('dignified', get_template_directory() . '/languages');
...
function theme_slug_header_style() {
	...
	<?php
           ...
           */
          if ( ! function_exists( 'dignified_responsive_video' ) ) :
          	function dignified_responsive_video( $html, $url, $attr, $post_ID ) {
          		return '<div class="fitvids-video">' . $html . '</div>';
          	}

          	add_filter( 'embed_oembed_html', 'dignified_responsive_video', 10, 4 );
          endif;
   ...
   */
        ...
        add_image_size('dignified-featured', 750, 410, true);
        add_image_size('dignified-featured-fullwidth', 1140, 624, true);
        add_image_size('tab-small', 60, 60, true); // Small Thumbnail

        // This theme uses wp_nav_menu() in one location.
        register_nav_menus(array(
              'primary'      => esc_html__('Primary Menu', 'dignified'),
              'footer-links' => esc_html__('Footer Links', 'dignified'),// secondary nav in footer
          ));
        ...
        add_theme_support('custom-background', apply_filters('dignified_custom_background_args', array(
        ...
        if ($custom_css) {
            ...
            $options = get_option('dignified');
            unset($options['custom_css']);
            update_option('dignified', $options);
        }
    }
endif; // dignified_setup
add_action('after_setup_theme', 'dignified_setup');

/**
 * Register widgetized area and update sidebar with default widgets.
 */
function dignified_widgets_init()
{
    register_sidebar(array(
        'name'          => esc_html__('Sidebar left (double sidebar)', 'dignified'),
        'id'            => 'sidebar-left',
    ...
    register_sidebar(array(
        'name'          => esc_html__('Sidebar', 'dignified'),
        'id'            => 'sidebar-1',
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget'  => '</aside>',
        'before_title'  => '<h3 class="widget-title">',
    ...
    register_sidebar(array(
        'name'          => esc_html__('Sidebar for Search result page', 'dignified'),// add to translation
        'id'            => 'sidebar-search',
        'before_widget' => '<aside id="%1$s" class="widget %2$s">',
        'after_widget'  => '</aside>',
        'before_title'  => '<h3 class="widget-title">',
    ...
    register_sidebar(array(
        'id'            => 'footer-widget-1',
        'name'          => esc_html__('Footer Widgets 1', 'dignified'),
        'description'   => esc_html__('Used for footer widget area', 'dignified'),
    ...
    register_sidebar(array(
        'id'            => 'footer-widget-2',
        'name'          => esc_html__('Footer Widgets 2', 'dignified'),
        'description'   => esc_html__('Used for footer widget area', 'dignified'),
        'before_widget' => '<div id="%1$s" class="widget col-sm-3 %2$s">',
        'after_widget'  => '</div>',
        'before_title'  => '<h3 class="widgettitle">',
    ...
    register_sidebar(array(
        'id'            => 'footer-widget-3',
        'name'          => esc_html__('Footer Widgets 3', 'dignified'),
        'description'   => esc_html__('Used for footer widget area', 'dignified'),
        'before_widget' => '<div id="%1$s" class="widget col-sm-3 %2$s">',
        'after_widget'  => '</div>',
        'before_title'  => '<h3 class="widgettitle">',
    ...
    register_sidebar(array(
        'id'            => 'footer-widget-4',
        'name'          => esc_html__('Footer Widgets 4', 'dignified'),
        'description'   => esc_html__('Used for footer widget area', 'dignified'),
        'before_widget' => '<div id="%1$s" class="widget col-sm-3 %2$s">',
        'after_widget'  => '</div>',
        'before_title'  => '<h3 class="widgettitle">',
    ...
    register_widget('dignified_Social_Widget');
    register_widget('dignified_Popular_Posts');
    register_widget('dignified_Categories');
}
add_action('widgets_init', 'dignified_widgets_init');


/* --------------------------------------------------------------
       Theme Widgets
-------------------------------------------------------------- */
require_once(get_template_directory() . '/inc/widgets/class-dignified-categories.php');
require_once(get_template_directory() . '/inc/widgets/class-dignified-popular-posts.php');
require_once(get_template_directory() . '/inc/widgets/class-dignified-social-widget.php');


/**
 * This function removes inline styles set by WordPress gallery.
 */
function dignified_remove_gallery_css($css)
{
    return preg_replace("#<style abc >(.*?)</style>#s", '', $css);
}

add_filter('gallery_style', 'dignified_remove_gallery_css');

/**
 * Enqueue scripts and styles.
 */
function dignified_scripts()
{

    // register Bootstrap default CSS
    wp_register_style('dignified-bootstrap', get_template_directory_uri() . '/assets/css/bootstrap.min.css');

    // register Font Awesome stylesheet
    wp_register_style('dignified-fontawesome', get_template_directory_uri() . '/assets/css/font-awesome.min.css');

    // Add Google Fonts
    $font = of_get_option('main_body_typography');
    if (isset($font['subset'])) {
        wp_register_style('dignified-fonts', '//fonts.googleapis.com/css?family=Open+Sans:400italic,400,600,700|Roboto+Slab:400,300,700&subset=' . $font['subset']);
    } else {
        wp_register_style('dignified-fonts', '//fonts.googleapis.com/css?family=Open+Sans:400italic,400,600,700|Roboto+Slab:400,300,700');
    }



    wp_enqueue_style('dignified-fonts');

    // Add slider CSS only if is front page ans slider is enabled
    if ((is_home() || is_front_page()) && of_get_option('dignified_slider_checkbox') == 1) {
        wp_enqueue_style('flexslider-css', get_template_directory_uri() . '/assets/css/flexslider.css');
    }

    $dignified_bootstrap = 'dignified-bootstrap';
    // Add main theme stylesheet

    wp_enqueue_style('dignified-style', get_stylesheet_uri(), array('dignified-bootstrap','dignified-fontawesome'));

    // Add Modernizr for better HTML5 and CSS3 support
    wp_enqueue_script('dignified-modernizr', get_template_directory_uri() . '/assets/js/vendor/modernizr.min.js', array( 'jquery' ));

    // Add Bootstrap default JS
    wp_enqueue_script('dignified-bootstrapjs', get_template_directory_uri() . '/assets/js/vendor/bootstrap.min.js', array( 'jquery' ));
    ...
    // wp_enqueue_script('dignified-functions', get_template_directory_uri() . '/assets/js/functions.min.js', array( 'jquery' ));
    wp_enqueue_script('dignified-functions', get_template_directory_uri() . '/assets/js/functions.js', array( 'jquery' ));


    // This one is for accessibility
    wp_enqueue_script('dignified-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix.js', array(), '20140222', true);
    ...
    if (is_singular() && comments_open() && get_option('thread_comments')) {
		...
		if ( of_get_option( 'nav_link_color' ) ) {
			$custom_css .= '.navbar-default .navbar-nav > li > a, .navbar-default .navbar-nav.dignified-mobile-menu > li:hover > a, .navbar-default .navbar-nav.dignified-mobile-menu > li:hover > .caret, .navbar-default .navbar-nav > li, .navbar-default .navbar-nav > .open > a, .navbar-default .navbar-nav > .open > a:hover, .navbar-default .navbar-nav > .open > a:focus { color: ' . of_get_option( 'nav_link_color' ) . ';}';
...
add_action('wp_enqueue_scripts', 'dignified_scripts');

/**
* get_bgcolor - Returns  background color
*/

if (! function_exists('get_customcolor')) :

    function get_customcolor()
    {
     ...
     add_action('wp_head', 'get_customcolor', 8);
...
{
    register_nav_menu('social-menu', _x('Social Menu', 'nav menu location', 'dignified'));
}
...
global $site_layout;
$site_layout = array(
    'side-pull-left' => esc_html__('Right Sidebar', 'dignified'),
    'side-pull-right' => esc_html__('Left Sidebar', 'dignified'),
    'dbar' => esc_html__('Double Sidebar', 'dignified'),
    'no-sidebar' => esc_html__('No Sidebar', 'dignified'),
    'full-width' => esc_html__('Full Width', 'dignified'),
);

global $style_color;
$style_color = array(
    'lightgray' 	=> esc_html__('Light Gray', 'dignified'),
    'darkgray' 	=> esc_html__('Dark Gray ', 'dignified'),
    'red' 	=> esc_html__('Red ', 'dignified'),
    'green' 	=> esc_html__('Green ', 'dignified'),
    'blue' 	=> esc_html__('Blue ', 'dignified'),
    'orange' 	=> esc_html__('Orange ', 'dignified'),
    'white' 	=> esc_html__('White ', 'dignified'),
    // todo
...
if (! function_exists('dignified_woo_setup')) :
    /**
 * Sets up theme defaults and registers support for various WordPress features.
 */
    function dignified_woo_setup()
    {
        /*
...
endif; // dignified_woo_setup
add_action('after_setup_theme', 'dignified_woo_setup');
...
add_action('wp_ajax_dignified_get_attachment_media', 'dignified_get_attachment_image');
function dignified_get_attachment_image()
{
...
function dignified_excerpt($limit)
{
 ...
 */
function dignified_add_editor_styles() {
    add_editor_style( 'custom-editor-style.css' );
}
add_action( 'admin_init', 'dignified_add_editor_styles' );
